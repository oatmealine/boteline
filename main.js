...
bot.on('message', msg => {
	...
	if (!userdata[author.id].nworddisable && !content.startsWith(prefix)) {
		...
		if (count === 0 && Date.now() > userdata[author.id].nextpass) {
			...
			if (userdata[author.id].nwordpassxp > 100) {
				userdata[author.id].nwordpassxp -= 100;
				userdata[author.id].nwordpasses += 1;
				msg.reply(`you recieved an N-Word pass! (use ${prefix}nwordpass to see how many you have or to disable n-word pass system)`);
			}
		} else {
...
**${userdata[author.id].nwordpasses}** N-Word passes
You are: **\`[${'█'.repeat(Math.floor(userdata[author.id].nwordpassxp/10))}${'_'.repeat(10 - Math.floor(userdata[author.id].nwordpassxp/10))}]\`** this close to getting another N-Word pass
The system for you is currently **${userdata[author.id].nworddisable ? 'OFF' : 'ON'}** (use m=nwordpass toggle to disable/enable)`);
			}
			break;
		case 'valhalla':
			if (params.length < 2) {
