...
if (process.env.DEBUG) {
    console.debug(ch.grey('debug printing on'));
}
...
cs.addCommand('fun', new cs.Command('valhalla', (msg) => {
    ...
    if (params[0] === 'search') {
        ...
        let drinkbig;
        valhalladrinks.forEach((d) => {
            if (adelhyde + bronson_extract + powdered_delta + flangerine + karmotrine > 20) {
                return;
            }
            ...
            if (adelhyde !== d.ingredients.adelhyde && (adelhyde / 2 !== d.ingredients.adelhyde || !drinkbig)) {
                return;
            }
            if (bronson_extract !== d.ingredients.bronson_extract && (bronson_extract / 2 !== d.ingredients.bronson_extract || !drinkbig)) {
                return;
            }
            if (powdered_delta !== d.ingredients.powdered_delta && (powdered_delta / 2 !== d.ingredients.powdered_delta || !drinkbig)) {
                return;
            }
            if (flangerine !== d.ingredients.flangerine && (flangerine / 2 !== d.ingredients.flangerine || !drinkbig)) {
                return;
            }
            if ((karmotrine !== d.ingredients.karmotrine && (karmotrine / 2 !== d.ingredients.karmotrine || !drinkbig)) && d.ingredients.karmotrine !== 'optional') {
                return;
            }
            if (blended !== d.blended) {
                return;
            }
            if (aged !== d.aged) {
                return;
...
cs.addCommand('fun', new cs.Command('foxquote', (msg) => {
    const randommsg = Object.values(foxquotes)[Math.floor(Math.random() * foxquotes.length)];
